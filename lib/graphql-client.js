...
/**
 * Executes a GraphQL query against the given endpoint.
 * @param {string} endpoint - GraphQL endpoint URL
 * @param {object} options - Query options: { query, variables, operationName }
 * @param {function} [httpClient] - HTTP client (default: axios)
 * @returns {Promise<object>} GraphQL response object
 * @throws {Error} If endpoint is not a non-empty string or HTTP error occurs
 */
const executeQuery = async (
  ...
  require("./asserts").assertString(endpoint, "endpoint");
  if (typeof query !== "string" || !query.trim()) {
    throw new Error("executeQuery: query must be a non-empty string");
  }
  try {
